...
from agent.config import INGEST_MAX_OUTPUT_TOKENS

from rag.ingestor import _call_deepseek_for_ingest

def fake_llm(msgs):
    # Imprime lo que se manda, devuelve "" para forzar fallback
    print("=== PROMPT ===")
    print(msgs[-1]["content"][:500])
    return ""

agent = IngestAgent(llm_call=_call_deepseek_for_ingest, fallback_enabled=True)
out = agent.process("""---
| key | value |
|--|--|
...
**Mantenedor:** David
""", r"C:\Users\ochoa\standardtooling\theChat\copiaDelindexGlobalCogneu.md")
print(out)